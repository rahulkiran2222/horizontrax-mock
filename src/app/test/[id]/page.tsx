    ...
    return (
      <div className="fixed inset-0 bg-slate-900 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-3xl max-w-2xl w-full p-8 md:p-12 shadow-2xl overflow-y-auto max-h-[90vh]">
          ...
          <h2 className="text-3xl font-black text-slate-800 mb-2">Mock Test Instructions</h2>
          <ul className="space-y-4 text-slate-600 mb-10 text-lg">
            <li>1. Total Questions: <span className="font-bold text-slate-900">30 Questions</span></li>
            <li>2. Time Duration: <span className="font-bold text-slate-900">30 Minutes</span></li>
            <li className="text-red-500 font-medium italic">3. Strict Proctoring: No tab switching allowed.</li>
          </ul>
    ...
    <div className="h-screen bg-[#F0FDF4] flex flex-col font-sans text-slate-900 overflow-hidden">
      <header className="h-16 bg-white border-b px-6 flex justify-between items-center shrink-0">
        ...
        <div className="flex items-center gap-8">
            <div className="bg-green-50 text-green-700 px-4 py-1 rounded-full text-xs font-bold border border-green-100">Question {currentIdx + 1} of 30</div>
            <div className="text-slate-600 font-mono text-sm">Time: <span className="font-bold text-slate-900">{Math.floor(timeLeft / 60)}:{String(timeLeft % 60).padStart(2, '0')}</span></div>
            <LogOut className="text-slate-400 cursor-pointer hover:text-red-500" size={20} onClick={submitExam} />
        </div>
      </header>
      <div className="flex flex-1 overflow-hidden">
        <aside className="w-72 bg-white border-r flex flex-col p-6 overflow-y-auto shrink-0 shadow-sm">
          <h3 className="font-bold text-slate-800 mb-6 text-sm uppercase tracking-widest">Question Palette</h3>
          <div className="grid grid-cols-4 gap-3">
            {test.questions.map((_, i) => (
              <button key={i} onClick={() => setCurrentIdx(i)} className={`h-12 rounded-xl text-sm font-bold transition-all border ${currentIdx === i ? "border-green-600 bg-green-600 text-white" : answers[i] ? "border-green-200 bg-green-50 text-green-600" : "border-slate-100 text-slate-400 bg-slate-50"}`}>{i + 1}</button>
            ))}
          </div>
        </aside>
        <main className="flex-1 p-8 md:p-16 overflow-y-auto">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800 mb-10">{test.questions[currentIdx].text}</h1>
            <div className="space-y-4">
              {test.questions[currentIdx].options.map((opt) => (
                <button key={opt} onClick={() => setAnswers({...answers, [currentIdx]: opt})} className={`w-full p-6 rounded-2xl border-2 transition-all text-left flex items-center gap-5 ${answers[currentIdx] === opt ? "border-green-600 bg-green-50 text-green-900" : "border-white bg-white hover:border-green-50 shadow-sm"}`}>
                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${answers[currentIdx] === opt ? "border-green-600 bg-green-600" : "border-slate-200"}`}>{answers[currentIdx] === opt && <div className="w-2 h-2 bg-white rounded-full" />}</div>
            ...
            <div className="mt-20 pt-10 border-t border-slate-200 flex justify-between items-center">
               <button onClick={() => setCurrentIdx(v => v - 1)} disabled={currentIdx === 0} className="text-slate-400 font-bold disabled:opacity-0">← Previous</button>
